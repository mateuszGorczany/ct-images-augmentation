...
from src.models.medical_diffusion.vq_gan_3d.utils import (
    shift_dim,
    adopt_weight,
)
...
from src.utils import (
    dicom_batch_to_video_batch,
    log_visualizations,
    video_batch_to_dicom_batch,
    log_common_metrics
)
...
class ModelParameters:
    ...
    disc_loss_type: str
    image_gan_weight: float
    video_gan_weight: float
    l1_weight: float
    gan_feat_weight: float
...
class VQGAN(pl.LightningModule):
    ...
    def step(self, x, optimizer_idx, log_image=False):
        ...
        if optimizer_idx == 0:
            ...
            # recon_loss = recon_loss + commitment_loss + aeloss + perceptual_loss + gan_feat_loss
            return (
                recon_loss,
                x_recon,
                vq_output,
    ...
    def training_step(self, batch):
        ...
        discloss = self.step(x, 1, False)
        self.manual_backward(discloss)
        optimizer_disc.step()
        self.untoggle_optimizer(optimizer_disc)
        torch.nn.utils.clip_grad_norm_(self.parameters(), 1.0)

        log_common_metrics(self, x, video_batch_to_dicom_batch(x_recon), "train")

    def validation_step(self, batch):
        x = batch
        recon_loss, x_recon, vq_output, perceptual_loss = self.step(x, 2)
        ...
        x_recon = video_batch_to_dicom_batch(x_recon)
        log_common_metrics(
            self, x, x_recon, "val",
        )
    ...
    #     x = batch
    #     x = x.to(self.device)
    #     frames, frames_rec, _, _ = self.step(x, 2, log_image=True)
    #     log["inputs"] = frames
    ...
    #     log = dict()
    #     x = batch
    #     _, _, x, x_rec = self.step(x, 2, log_image=True)
    #     log["inputs"] = x
