...
from src.utils import video_batch_to_dicom_batch, dicom_batch_to_video_batch, log_common_metrics, log_visualizations
...
class VQVAE(BasicModel):
    ...
    def _step(self, batch: torch.Tensor, batch_idx: int, state: str, step: int):
        # ------------------------- Get Source/Target ---------------------------
        x = dicom_batch_to_video_batch(batch)
        target = x
        ...
        # --------------------- Compute Metrics  -------------------------------
        pred = video_batch_to_dicom_batch(pred)
        x = video_batch_to_dicom_batch(x)
        log_common_metrics(self, x, pred, state)
        # ----------------- Save viz ------------------------------
        if self.current_epoch % self.sample_every_n_epochs == 0:
            log_visualizations(self, x, pred)
...
class VQGAN(VeryBasicModel):
    ...
    def forward(self, x_in):
        return self.vqvae.forward(x_in)
    ...
    def _step(self, batch: torch.Tensor, batch_idx: int, state: str, step: int):
        ...
        opt_vqvae, opt_gan = self.optimizers()
        if state == 'train':
            self.toggle_optimizer(opt_vqvae)
            opt_vqvae.zero_grad()
        ...
        if state == 'train':
            ...
            opt_vqvae.step()
            self.untoggle_optimizer(opt_vqvae)
        
        with torch.no_grad():
            logging_dict[f'img_loss'] = img_loss
            logging_dict[f'emb_loss'] = emb_loss
            logging_dict['loss_vae'] = loss

        # Horizontal/Top Layer
        if state == 'train':
            self.toggle_optimizer(opt_gan)
            opt_gan.zero_grad()
        ...
        if state == "train":
            self.manual_backward(loss)
            opt_gan.step()
            self.untoggle_optimizer(opt_gan)

        with torch.no_grad():
                logging_dict['loss_gan'] = loss
