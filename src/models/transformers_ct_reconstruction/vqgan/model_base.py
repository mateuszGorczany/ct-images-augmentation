...
# from pytorch_lightning.utities.cloud_io import load as pl_load
...
class VeryBasicModel(pl.LightningModule):
    ...
    def test_step(self, batch: torch.Tensor, batch_idx: int, optimizer_idx:int = 0):
        self._step_test += 1
    ...
    def load_pretrained(self, checkpoint_path, map_location=None, **kwargs):
        ...
        #     else:
        #         checkpoint = (checkpoint_path, map_location=lambda storage, loc: storage)
        # checkpoint = self.load_from_checkpoint(checkpoint_path, **kwargs)
